...
export class OnboardAuthorUseCase {
  ...
  async execute(
    ...
    const frontendUrl = this.configService.get<string>('APP_FRONTEND_URL', 'http://localhost:3000');
    const refreshPath = this.configService.get<string>('STRIPE_CONNECT_REFRESH_PATH', '/onboarding/refresh');
    const returnPath  = this.configService.get<string>('STRIPE_CONNECT_RETURN_PATH', '/onboarding/complete');
