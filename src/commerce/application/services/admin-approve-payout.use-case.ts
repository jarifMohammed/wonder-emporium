...
export class AdminApprovePayoutUseCase {
  ...
  async execute(payoutId: string) {
    ...
    if (payout.status !== 'REQUESTED') {
      throw AppError.badRequest(`Cannot approve payout in status: ${payout.status}`);
    }

    if (!payout.author.stripeAccount || !payout.author.stripeAccount.stripeAccountId) {
      throw AppError.badRequest('Author does not have a connected Stripe account');
    }

    if (!payout.author.stripeAccount.payoutsEnabled) {
      throw AppError.badRequest('Author Stripe account is not fully verified/enabled for payouts');
    }
    ...
    try {
      ...
      this.logger.log(`Payout ${payoutId} approved and transfer ${transfer.id} executed`);
      return updatedPayout;

    } catch (error: any) {
      this.logger.error(`Failed to execute Stripe transfer for payout ${payoutId}: ${error.message}`);
